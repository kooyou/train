...
update_userinfo(Data) ->
    {UserId,Name,LoginTimes,ChatTimes,LastTimes} = Data,
    SqlStr = io_lib:format("update user_info set login_times = ~p,chat_times = ~p where id = ~p ",[LoginTimes,ChatTimes,UserId]),
    mysql:fetch(?CHAT_DB,SqlStr).
